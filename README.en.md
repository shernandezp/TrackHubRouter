...
- **Five things must align when adding a provider**: the `ProtocolType` enum value (in TrackHubCommon), the assembly namespace `TrackHub.Router.Infrastructure.{Protocol}`, each reader's `Protocol` property, the `AppSettings:Protocols` entry (**in both Web and SyncWorker**), and the `ProviderCapabilityCatalog` entry. Also keep the portal's `protocolTypes.ts` in lockstep.
