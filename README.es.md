...
- **Cinco cosas deben alinearse al agregar un proveedor**: el valor del enum `ProtocolType` (en TrackHubCommon), el espacio de nombres del ensamblado `TrackHub.Router.Infrastructure.{Protocol}`, la propiedad `Protocol` de cada lector, la entrada `AppSettings:Protocols` (**tanto en Web como en SyncWorker**), y la entrada de `ProviderCapabilityCatalog`. Mantener también sincronizado el `protocolTypes.ts` del portal.
